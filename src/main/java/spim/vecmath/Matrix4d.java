...
public class Matrix4d implements java.io.Serializable, Cloneable
{
	...
	public double m33;
	/*
	 * double[] tmp = new double[16]; double[] tmp_rot = new double[9]; //
	 * scratch matrix double[] tmp_scale = new double[3]; // scratch matrix
	 */
	...
	final void invertGeneral( Matrix4d m1 )
	{
		...
		if ( !luDecomposition( tmp, row_perm ) )
		{
			...
			throw new SingularMatrixException(
					VecMathI18N.getString( "Matrix4d10" ) );
		}
			...
			for (j = 0; j < 4; j++)
			{
				...
				if ( imax < 0 )
				{
					throw new RuntimeException(
							VecMathI18N.getString( "Matrix4d11" ) );
				}
	...
	public final void transform( Tuple4f vec, Tuple4f vecOut )
	{
		...
		vecOut.w = (float) ( m30 * vec.x + m31 * vec.y + m32 * vec.z + m33
				* vec.w );
		vecOut.x = x;
		vecOut.y = y;
		vecOut.z = z;
	...
	public final void transform( Vector3f normal, Vector3f normalOut )
	{
		...
		normalOut.z = (float) ( m20 * normal.x + m21 * normal.y + m22
				* normal.z );
		normalOut.x = x;
		normalOut.y = y;
	}
